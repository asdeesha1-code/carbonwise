...
type TabName = "house.fill" | "clock.arrow.circlepath" | "person.2.fill" | "slider.horizontal.3" | "bolt.fill" | "gamecontroller.fill" | "globe.americas.fill" | "person.crop.circle.fill";

function TabIcon({ name, color, focused }: { name: TabName; color: string; focused: boolean }) {
  return <View style={[styles.iconShell, focused && styles.iconShellActive]}><IconSymbol size={21} name={name} color={color} /></View>;
}
...
export default function TabLayout() {
  ...
  return <Tabs screenOptions={{ headerShown: false, tabBarActiveTintColor: colors.primary, tabBarInactiveTintColor: colors.muted, tabBarButton: HapticTab, tabBarStyle: { paddingTop: 6, paddingBottom: bottomPadding, height: 53 + bottomPadding, backgroundColor: colors.background, borderTopColor: colors.border, borderTopWidth: 0.5 }, tabBarLabelStyle: { fontSize: 8, fontWeight: "800" }, tabBarItemStyle: { minWidth: 45 } }}>
    ...
    <Tabs.Screen name="time-machine" options={{ title: "Time", tabBarIcon: ({ color, focused }) => <TabIcon name="slider.horizontal.3" color={color} focused={focused} /> }} />
...
const styles = StyleSheet.create({ iconShell: { width: 29, height: 25, borderRadius: 13, alignItems: "center", justifyContent: "center" }, iconShellActive: { backgroundColor: "#EAF7FF", transform: [{ scale: 1.08 }] } });
